...
class JSNMobilizeTemplateHelper {
	...
	public function renderOpcity($cm = NULL,$preview,$cookieStyle,$cookieStl,$keyworld) {
					...
					if($pathImg !=''){
						if (strpos($pathImg,'http://') !== false) {
							...
							if($_SERVER['SERVER_PORT'] !== ''){
								$pathImg = 'http://' . $_SERVER['SERVER_NAME'] .':'.$_SERVER['SERVER_PORT']. JURI::root(true) .'/administrator/'. $pathImg;
							}else{
								$pathImg = 'http://' . $_SERVER['SERVER_NAME'] . JURI::root(true) .'/administrator/'. $pathImg;
							}
						}
			...
			if($cm ==1 && $keyworld == 'jsn_template'){
				...
				$str.= '<style>#'.str_replace("_", "-", $keyworld).' .row-fluid{position:relative}</style>';
			}
