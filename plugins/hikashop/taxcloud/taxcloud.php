...
class plgHikaShopTaxcloud extends JPlugin {
	...
	public function onAfterOrderUpdate(&$order,&$send_email){
		...
		$confirmed_statuses = $config->get('invoice_order_statuses','0');
		$cancelled_statuses = $config->get('cancelled_order_status','0');
		$confirmed_statuses = explode(",", $confirmed_statuses);
	...
	public function partialReturn($order, $all){
		...
		else{//returning all the products with their actual quantities
			foreach($order->products as $product){
