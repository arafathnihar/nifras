				...
				foreach ($cart->products as $p)
				{
					...
					if($group)
					{
						...
						foreach ($cart->products as $p2)
							...
							{
								$productprice += round($p2->prices[0]->price_value,2);
								$tax += round($p2->prices[0]->price_value_with_tax,2) - round($p2->prices[0]->price_value,2);
								$amountCalculated += round($p2->prices[0]->price_value,2);
								$optionalProdDesc .= $p2->product_name.',';
							}
					}
					$tax += round($p->prices[0]->price_value_with_tax,2) - round($p->prices[0]->price_value,2);
					$amountCalculated += round($p->prices[0]->price_value,2);
					$productprice += round($p->prices[0]->price_value,2);
					$item = array(
		...
		if (isset($element->ctrl))
		{
			...
			elseif($element->ctrl=='checkout' && $this->layout=='step' && $this->canDisplayButton($element))
			{
				...
				if (!empty($contenttable[0][0]))
				{
					...
					$html = str_replace('</tfoot>',$inserthtml,$html);
				}
				echo $html;
			}
		}
