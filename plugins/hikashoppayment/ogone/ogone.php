...
class plgHikashoppaymentOgone extends hikashopPaymentPlugin
{
	...
	function generateHash($vars, $passphrase, $hash_method, $type = 'in') {
		...
		$key = '';
		foreach($vars as $k => $v) {
			if(strlen($v) && !in_array(strtoupper($k), array('SHASIGN','OPTION','CTRL','TASK','NOTIF_PAYMENT','TMPL','ITEMID','HIKASHOP_FRONT_END_MAIN','VIEW','LANG','ORDER_ID'))) {
				if($type == 'out' && in_array(strtoupper($k), array('LANGUAGE', 'USER_RETURN')))
					continue;
