...
if($this->payment_params->type == 'iframe'){
	echo '<iframe src="https://payflowlink.paypal.com?MODE='.$mode.'&SECURETOKEN='.$this->vars['SECURETOKEN'].'&SECURETOKENID='.$this->vars['SECURETOKENID'].'" name="paypal_iframe" scrolling="no" width="'.$this->payment_params->width.'px" height="'.$this->payment_params->height.'px"></iframe>';
}else{ ?>
	<form id="hikashop_paypaladvanced_form" name="hikashop_paypaladvanced_form" action="https://payflowlink.paypal.com" method="post">
		<?php
			...
			$doc->addScriptDeclaration("window.addEvent('domready', function() {document.getElementById('hikashop_paypaladvanced_form').submit();});");
