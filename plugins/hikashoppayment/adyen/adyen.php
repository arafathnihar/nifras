...
?><?php
	...
	class plgHikashoppaymentadyen extends hikashopPaymentPlugin
	{
		...
		function onAfterOrderConfirm(&$order, &$methods, $method_id) {
			...
			{
				$this->app->enqueueMessage('You have to configure a merchant account for the adyen plugin payment first : check your plugin\'s parameters,
			...
			{
				$this->app->enqueueMessage('You have to configure the Skin Code for the adyen plugin payment first : check your plugin\'s parameters,
				on your website backend','error');
			...
			{
				$this->app->enqueueMessage('You have to configure the Hmac Key for the adyen plugin payment first : check your plugin\'s parameters,
				on your website backend','error');
				return false;
			}
			...
			else
			{
				...
				$vars = array(
					...
					'sessionValidity'=>date("c",strtotime("+".$this->payment_params->sessionvalidity." days")),
				...
				$vars['paymentAmount'] . $vars['currencyCode'] . $vars['shipBeforeDate'] . $vars['merchantReference'] . $vars['skinCode'] . $vars['merchantAccount'] .
				$vars['sessionValidity'] . $vars['shopperEmail'] . $vars['shopperReference'] .
				$vars['allowedMethods'] . $vars['blockedMethods'],
				trim($this->payment_params->hmacKey)
				)));

				$vars['billingAddressSig'] = base64_encode(pack("H*",hash_hmac('sha1',
				$vars['billingAddress.street'] .
				$vars['billingAddress.houseNumberOrName'] .
				$vars['billingAddress.city'] .
				$vars['billingAddress.postalCode'] .
				$vars['billingAddress.stateOrProvince'] .
				$vars['billingAddress.country'],
				trim($this->payment_params->hmacKey)
				)));

				$vars['deliveryAddressSig'] = base64_encode(pack("H*",hash_hmac('sha1',
				$vars['deliveryAddress.street'] .
				$vars['deliveryAddress.houseNumberOrName'] .
				$vars['deliveryAddress.city'] .
				$vars['deliveryAddress.postalCode'] .
				$vars['deliveryAddress.stateOrProvince'] .
				$vars['deliveryAddress.country'],
				trim($this->payment_params->hmacKey)
				)));

				$vars['shopperSig'] = base64_encode(pack("H*",hash_hmac('sha1',
				$vars['shopper.firstName'] .
				$vars['shopper.lastName'] .
				$vars['shopper.telephoneNumber'],
				trim($this->payment_params->hmacKey)
				)));
		...
		function splitStreet($street){
			if (preg_match("#[0-9]+#",$street,$matches)){
				...
				$array=array($matches[0],preg_replace('#'.$matches[0].'#','',$street,1));
				return $array;
			}
			else {
				return array('',$street);
		...
		function onPaymentNotification(&$statuses)
		{
			...
			{
				$hmacKey =base64_encode(pack("H*",hash_hmac('sha1',$vars['authResult'].$vars['pspReference'].$vars['merchantReference'].$vars['skinCode'],
					trim($this->payment_params->hmacKey)
					)));
			...
			if($vars['authResult']!='PENDING'&& $vars['authResult']!='AUTHORISED') //set the "invalid" status
			{
				if($this->payment_params->debug)
			...
			else //validated -> success
			{
				...
				if(!empty($order_status))$this->modifyOrder($order_id, $order_status, true, true);
