...
class plgHikashopshippingUSPS extends hikashopShippingPlugin
{
	...
	function onShippingDisplay(&$order,&$dbrates,&$usable_rates,&$messages){
		...
		foreach($local_usable_rates as $rate) {
			if($rate->shipping_type=='usps'){
				...
				foreach($order->products as $k => $product){
					...
					if(!empty($product->cart_product_quantity)){
						if (!empty($product->product_height))
							$package_height = $volumeClass->convert($product->product_height, $product->product_dimension_unit_orig, 'in' , 'dimension') * $product->cart_product_quantity;
						if (!empty($product->product_length))
							$package_lenght = $volumeClass->convert($product->product_length, $product->product_dimension_unit_orig, 'in', 'dimension' ) * $product->cart_product_quantity;
						if (!empty($product->product_width))
							$package_width = $volumeClass->convert($product->product_width, $product->product_dimension_unit_orig, 'in', 'dimension' ) * $product->cart_product_quantity;
					}
				}
				...
				if(empty($parcel->Country)) $parcel->Country='US';
				...
				$parcel->Length=$package_lenght;
				$parcel->Width=$package_width;
				$parcel->Height=$package_height;
				$parcel->Girth= (($package_height + $package_width)*2) + $package_lenght;
				$parcel->Quantity=1;
				...
				if($rate->shipping_params->sort_by_price == 'DESC')
					$rates = array_sort($rates, 'shipping_price', 'usps', SORT_DESC);
				if($rate->shipping_params->sort_by_price == 'ASC')
					$rates = array_sort($rates, 'shipping_price', 'usps', SORT_ASC);
				$i=0;
	...
	function addRate(&$rates,$type,$parcel,&$rate,$currency, $isInternational){
		...
		if($isInternational) {
			...
			$request .= '<Container>' . $rate->shipping_params->container . '</Container>';
			$request .= '<Size>Regular</Size>';
			$request .= '<Width>'.$parcel->Width.'</Width>';
			$request .= '<Length>'.$parcel->Length.'</Length>';
			$request .= '<Height>'.$parcel->Height.'</Height>';
			$request .= '<Girth>'.$parcel->Girth.'</Girth>';
			$request .= '</Package>';
