...
class plgHikashopshippingCANADAPOST extends hikashopShippingPlugin {
	...
	function _getShippingMethods(&$rate, &$order, &$warehouse, $null) {
		...
		if (!$rate -> shipping_params -> group_package || $rate -> shipping_params -> group_package == 0) {
            ...
            $l_height = 0;
            $l_width = 0;
			if ((!empty($rate -> shipping_params -> dim_approximation_l) && isset($rate -> shipping_params -> dim_approximation_l)))
                $l_length = $rate -> shipping_params -> dim_approximation_l;
            if ((!empty($rate -> shipping_params -> dim_approximation_l) && isset($rate -> shipping_params -> dim_approximation_kg)))
                $l_weight = $rate -> shipping_params -> dim_approximation_kg;
            if ((!empty($rate -> shipping_params -> dim_approximation_l) && isset($rate -> shipping_params -> dim_approximation_h)))
                $l_height = $rate -> shipping_params -> dim_approximation_h;
            if ((!empty($rate -> shipping_params -> dim_approximation_l) && isset($rate -> shipping_params -> dim_approximation_w)))
