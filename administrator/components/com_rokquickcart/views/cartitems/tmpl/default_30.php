<?php
/**
 * @version   $Id: default_30.php 19336 2014-02-28 20:30:52Z djamil $
...
$canOrder = $user->authorise('core.edit.state', 'com_rokquickart.category');
...
<form action="<?php echo JRoute::_('index.php?option=com_rokquickcart'); ?>" method="post" name="adminForm"
      id="adminForm">
			...
			<div id="filter-bar" class="btn-toolbar">
				<div class="filter-search btn-group pull-left">
					...
					<input type="text" name="filter_search" id="filter_search"
					       placeholder="<?php echo JText::_('ROKQUICKCART_SEARCH_IN_NAME'); ?>"
					       value="<?php echo $this->escape($this->state->get('filter.search')); ?>"
					       title="<?php echo JText::_('ROKQUICKCART_SEARCH_IN_NAME'); ?>"/>
				</div>
					...
					<tr class="row<?php echo $i % 2; ?>" sortable-group-id="1">
						...
						<td class="center">
							<div class="btn-group">
								...
								JHtml::_('actionsdropdown.' . $action, 'cb' . $i, 'cartitems');
						...
						<td class="nowrap has-context">
							<div class="pull-left">
								...
								<?php if ($canEdit || $canEditOwn) : ?>
									<a href="<?php echo JRoute::_('index.php?option=com_rokquickcart&task=cartitem.edit&id=' . (int)$item->id); ?>">
							...
							<div class="pull-left">
								<?php
								// Create dropdown items
								JHtml::_('dropdown.edit', $item->id, 'cartitem.');
								JHtml::_('dropdown.divider');
								if ($item->published) :
									JHtml::_('dropdown.unpublish', 'cb' . $i, 'cartitems.'); else :
									JHtml::_('dropdown.publish', 'cb' . $i, 'cartitems.');
								endif;

								JHtml::_('dropdown.divider');

								if ($archived) :
									JHtml::_('dropdown.unarchive', 'cb' . $i, 'cartitems.'); else :
									JHtml::_('dropdown.archive', 'cb' . $i, 'cartitems.');
								endif;

								if ($item->checked_out) :
									JHtml::_('dropdown.checkin', 'cb' . $i, 'cartitems.');
								endif;

								if ($trashed) :
									JHtml::_('dropdown.untrash', 'cb' . $i, 'cartitems.'); else :
									JHtml::_('dropdown.trash', 'cb' . $i, 'cartitems.');
								endif;
